"""SNI Spoofer – Windows GUI v3.0

Connection modes
~~~~~~~~~~~~~~~~
* **SNI Only**          – Xray (Trojan) tunnelled through the SNI spoofer.
* **SNI + Warp**        – Vwarp (basic WARP) tunnelled through the SNI spoofer.
* **SNI + Psiphon**     – Vwarp (WARP + Psiphon) tunnelled through the SNI spoofer.
* **SNI + Warp-in-Warp**– Vwarp (gool / double-hop) tunnelled through the SNI spoofer.
* **Gaming**            – Same as *SNI Only* with aggressive low-latency tuning.

All core binaries (xray, vwarp) are bundled inside the exe.
"""
import ctypes
import os
import sys
...
from core.vwarp_manager import VwarpManager                          # noqa: E402
from transparent_spoof import TransparentSpoofServer                 # noqa: E402
...
PSIPHON_COUNTRIES = [
    "US", "DE", "NL", "GB", "FR", "CA", "JP", "SG", "AU", "AT",
    "BE", "BG", "BR", "CH", "CZ", "DK", "EE", "ES", "FI", "HK",
    "HR", "HU", "IE", "IN", "IT", "LV", "NO", "PL", "PT", "RO",
    "RS", "SE", "SK", "UA",
]
...
class SNIProxyGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("SNI Spoofer v3.0")
        ...
        self.vwarp: VwarpManager | None = None
        self.tspoof: TransparentSpoofServer | None = None
    ...
    def _build_ui(self):
        ...
        ttk.Label(hdr, text="\U0001f6e1  SNI Spoofer v3.0",
    ...
    def _build_settings(self, parent):
        ...
        # ── Warp / Vwarp Settings ─────────────────────────────────────────
        self.warp_frame = ttk.LabelFrame(sf, text="Warp / Vwarp Settings")
        ...
        # ── Psiphon Settings ─────────────────────────────────────────────
        self.psiphon_frame = ttk.LabelFrame(sf, text="Psiphon Settings")
        self.psiphon_frame.columnconfigure(1, weight=1)

        ttk.Label(self.psiphon_frame, text="Country:").grid(
            row=0, column=0, padx=(10, 4), pady=3, sticky="w")
        self.psiphon_country_var = tk.StringVar(value="US")
        pc = ttk.Combobox(self.psiphon_frame, textvariable=self.psiphon_country_var,
                          values=PSIPHON_COUNTRIES, state="readonly", width=8)
        pc.grid(row=0, column=1, padx=(0, 10), pady=3, sticky="w")

        ttk.Label(self.psiphon_frame, text="Warp Endpoint:").grid(
            row=1, column=0, padx=(10, 4), pady=3, sticky="w")
        self.psiphon_ep_var = tk.StringVar()
        pep = ttk.Combobox(self.psiphon_frame, textvariable=self.psiphon_ep_var,
                           values=WARP_ENDPOINTS, width=22)
        pep.grid(row=1, column=1, padx=(0, 10), pady=3, sticky="w")

        ttk.Label(self.psiphon_frame, text="License Key:").grid(
            row=2, column=0, padx=(10, 4), pady=3, sticky="w")
        self.e_psiphon_license = ttk.Entry(self.psiphon_frame, width=38)
        self.e_psiphon_license.grid(row=2, column=1, padx=(0, 10), pady=3, sticky="ew")
    ...
    def _build_log(self, parent):
        self.log_text = scrolledtext.ScrolledText(
            ...
            selectbackground=self.ACCENT, selectforeground="#1e1e2e")
        # i want can copy text with ctrl+c selected text 
    ...
    def _load_config(self):
        ...
        self.psiphon_country_var.set(c.get("psiphon_country", "US"))
        self.psiphon_ep_var.set(     c.get("psiphon_endpoint", "162.159.192.1"))
        _set(self.e_psiphon_license, c.get("psiphon_license", ""))
    ...
    def _read_config(self) -> dict:
        ...
        return {
            ...
            "psiphon_country": self.psiphon_country_var.get(),
            "psiphon_endpoint": self.psiphon_ep_var.get().strip(),
            "psiphon_license": self.e_psiphon_license.get().strip(),
        }
    ...
    def _on_mode_change(self, *_args):
        ...
        # Hide optional frames first
        self.v2_frame.pack_forget()
        self.warp_frame.pack_forget()
        self.psiphon_frame.pack_forget()

        # V2Ray (Xray) is always needed – it creates the base tunnel
        self.v2_frame.pack(fill="x", padx=10, pady=4, after=self._dpi_frame)

        # Show mode-specific frame after V2Ray
        if mode in ("SNI + Warp", "SNI + Warp-in-Warp"):
            self.warp_frame.pack(fill="x", padx=10, pady=4, after=self.v2_frame)
        elif mode == "SNI + Psiphon":
            self.psiphon_frame.pack(fill="x", padx=10, pady=4, after=self.v2_frame)
    ...
    def _start(self):
        ...
        uses_vwarp = mode in ("SNI + Warp", "SNI + Psiphon", "SNI + Warp-in-Warp")

        if uses_vwarp:
            # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            # Warp / Psiphon / Gool chain:
            #   Vwarp connects directly to Cloudflare IPs.
            #   TransparentSpoofServer intercepts ALL outgoing TCP:443
            #   and injects fake ClientHello so DPI can't block them.
            #   NO Xray, NO relay proxy – completely transparent.
            #
            #   Vwarp → (direct TCP:443) → WinDivert inject → Internet
            # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            self.tspoof = TransparentSpoofServer(fake_sni=cfg["FAKE_SNI"])
            self.tspoof.on_log = self._append_log
            self.tspoof.on_status_change = self._set_ui_running
            self.tspoof.start()

            import time
            # Small delay to let WinDivert bind
            def _start_vwarp_after_spoof():
                time.sleep(1)
                if mode == "SNI + Warp":
                    vwarp_mode = "warp"
                    endpoint = cfg["warp_endpoint"] + ":443"
                    license_key = cfg["warp_license"]
                elif mode == "SNI + Psiphon":
                    vwarp_mode = "psiphon"
                    endpoint = cfg.get("psiphon_endpoint", cfg["warp_endpoint"]) + ":443"
                    license_key = cfg.get("psiphon_license", "")
                else:  # SNI + Warp-in-Warp
                    vwarp_mode = "gool"
                    endpoint = cfg["warp_endpoint"] + ":443"
                    license_key = cfg["warp_license"]

                self.vwarp = VwarpManager(
                    bind_port=cfg["socks_port"],
                    endpoint=endpoint,
                    license_key=license_key,
                    mode=vwarp_mode,
                    psiphon_country=cfg.get("psiphon_country", "US"),
                )
                self.vwarp.on_log = self._append_log
                self.vwarp.start()
            threading.Thread(target=_start_vwarp_after_spoof, daemon=True).start()

            self._append_log(f"Mode: {mode}")
            self._append_log(
                "Chain: Vwarp → (direct) → Transparent SNI Spoof (all TCP:443) → Internet")
            self._append_log(
                f"User proxy  \u2192  SOCKS5 127.0.0.1:{cfg['socks_port']}")

        else:
            # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            # SNI Only / Gaming chain:
            #   ProxyServer (relay) on :40443 → CONNECT_IP
            #   Xray connects Trojan through the relay, exposes SOCKS/HTTP
            #
            #   User → Xray (SOCKS5) → ProxyServer → SNI Spoof → Internet
            # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            if not cfg["trojan_password"]:
                messagebox.showerror("Config Error", "V2Ray password is required")
                return

            # Start ProxyServer (relay proxy with SNI spoofing)
            proxy_cfg = {
    ...
    def _on_close(self):
        running = (
            (self.server and self.server.is_running)
            or (self.tspoof and self.tspoof.is_running)
        )
        if running:
            self._stop()
