...
## ✅ رفع بازخورد دوم (هنوز به‌هم‌ریخته + کانفیگ کار نمی‌کند + کندی) (2026-05-30)
**چرا:** کاربر نسخهٔ بیلدشده را دوباره تست کرد: (الف) UI هنوز در پنجرهٔ کوچک به‌هم می‌ریخت (فیلدها روی هم، متن نصفه)، (ب) ویجت‌های throughput و probe کاملاً **سفید** بودند، (ج) کانفیگ هنوز وصل نمی‌شد و **کندتر** از V2RayTun شده بود.

### رفع به‌هم‌ریختگی UI — اسکرول‌اِریا (ریشهٔ اصلی)
- **هیچ `QScrollArea` در صفحات نبود** → وقتی پنجره کوتاه بود، Qt ویجت‌ها را روی هم فشرده می‌کرد (همان «فیلدهای له‌شده/متن نصفه»). تابع `_scrollable(page)` افزوده شد و **همهٔ صفحات** در یک `QScrollArea` (resizable، viewport شفاف) پیچیده شدند؛ نگاشت `self._scroll[page]` اضافه شد تا منطق تعویض صفحه (`_on_page_changed`) همچنان درست کار کند.
- `_labelled_spin` بازنویسی شد: حذف `setMinimumHeight(74)` ثابت + `addStretch` که اسپین‌باکس را می‌فشرد؛ حالا VBox ارتفاع را از محتوا می‌گیرد.

### رفع ویجت‌های سفید (تم تیره اعمال نمی‌شد)
- **ریشه:** ویجت `tbl` در صفحهٔ تشخیص یک `QPlainTextEdit` با objectName=`Log` بود، ولی قانون QSS فقط `QTextEdit#Log` بود (نوع غلط) → سفید می‌ماند. `QProgressBar` هم **اصلاً استایل نداشت** → نوار سفید پیش‌فرض ویندوز.
- **رفع (`ui/theme.py`):** قانون پایهٔ `QTextEdit, QPlainTextEdit` (هیچ‌وقت سفید نشود) + `QTextEdit#Log, QPlainTextEdit#Log` + استایل کامل `QProgressBar`/`::chunk` با گرادیان accent→success.

### رفع کانفیگ کار نمی‌کند + کندی (مهم‌ترین)
- **ریشه:** در همهٔ حالت‌های دارای هسته، اسپوفر **زیر** xray زنجیر می‌شد و بسته‌های TLS/WS خودِ xray را دستکاری (DPI-bypass) می‌کرد. برای کانفیگ‌های Cloudflare/WS این کار هندشیک تونل را خراب یا **بسیار کند** می‌کرد.
- **رفع (`core/engine.py`):** پراپرتی جدید `chains_spoofer`. حالت سادهٔ **«Tunnel»** دیگر اسپوفر را زنجیر نمی‌کند؛ متد جدید `_start_core_only()` فقط xray را اجرا می‌کند و **مستقیم** به سرور وصل می‌شود (`spoof_port=None`) — دقیقاً مثل V2RayTun (سریع و پایدار). فقط ترکیب‌های صریح SNI («SNI + Warp/Psiphon/…») اسپوفر را زیر xray زنجیر می‌کنند چون عمداً DPI-evasion روی اتصال بیرونی می‌خواهند.
- پیش‌فرض‌های باقی‌مانده‌ی «SNI Only» در UI به «Tunnel» اصلاح شد (`load_from`، `_save_settings`، کارت آمار حالت، مقداردهی داشبورد).

- تست‌ها: `tests/test_mode_fix_ui.py` (+۳: `chains_spoofer` برای Tunnel/SNI-combo/SNI-Only) + `tests/test_engine.py` (+۱: Tunnel سادهٔ xray-مستقیم بدون اسپوفر). مجموعاً **۳۲۵ تست سبز + ۲ skip**. همهٔ صفحات (تنظیمات/تشخیص/پروفایل/استراتژی) در سایز واقعی پنجره (۱۰۲۴×۶۸۰) به‌صورت بصری تأیید شدند — بدون هم‌پوشانی و بدون ویجت سفید.

## ✅ رفع بازخورد سوم — ریشهٔ واقعی «کانفیگ داخلی وصل نمی‌شود» + تمیزکاری لیست پروفایل (2026-05-30)
**چرا:** کاربر لاگ کاملِ xray + فایل `xray_config.json` نسخهٔ بیلدشده را فرستاد و فرضیهٔ درستی داد: «هستهٔ ایکس‌ری روی پورت 40443 فعال نمی‌شود، ولی V2RayTun روی 40443 فعال می‌شود و کار می‌کند.» سپس نکتهٔ کلیدی را اضافه کرد: هنگام فعال‌شدن نرم‌افزار یک **پورت لوکال روی 40443** باز می‌شود و کانفیگ‌هایی مثل `vless://...@127.0.0.1:40443?...host=...workers.dev` با V2RayTun پینگ مثبت می‌گیرند — به‌شرطی که **میکس‌پورت V2RayTun = 10808** باشد (برابر پورت گوش‌دادن نرم‌افزار ما).

### ریشهٔ واقعی (تشخیص اصلاح‌شده)
- این کانفیگ‌ها (سبک «webtun»/CDN) **عمداً** به `127.0.0.1:40443` اشاره می‌کنند؛ آدرس placeholder غلط نیست. مقصد واقعی، دامنهٔ CDN است که در `sni`/`host` حمل می‌شود (`*.workers.dev` / `*.pages.dev` پشت Cloudflare). V2RayTun کار می‌کند چون **هلپر داخلی خودش را روی 40443** اجرا می‌کند که ترافیک را به CDN واقعی فوروارد می‌کند.
- باگ ما: در حالت Tunnel، اصلاح بازخورد دوم (`_start_core_only` با `spoof_port=None`) **هیچ اسپوفری روی 40443 اجرا نمی‌کرد** → xray به `127.0.0.1:40443` خالی وصل می‌شد → `connectex: actively refused`. (فرض اولیهٔ «بیلد قدیمی» اشتباه بود؛ این یک باگ واقعی است.)

### رفع (`core/profile.py`, `core/engine.py`, `core/xray_manager.py`)
- `core/profile.py`: تابع `_is_local_addr()` + پراپرتی‌های `is_webtun`, `upstream_address`, `upstream_port`. کانفیگ webtun وقتی شناسایی می‌شود که آدرس loopback باشد ولی `sni`/`host` یک دامنهٔ واقعی داشته باشد. مقصد واقعی = آن دامنه روی پورت ۴۴۳ (یا ۸۰ برای حالت بدون TLS).
- `core/engine.py`: `chains_spoofer` حالا برای کانفیگ‌های webtun **حتی در حالت Tunnel** نیز True است (اسپوفر باید روی همان پورت placeholder بنشیند). در مسیر زنجیره، برای webtun اسپوفر روی **پورت دقیق placeholder (40443)** گوش می‌دهد و به CDN واقعی فوروارد می‌کند؛ دامنهٔ CDN با `_resolve_ipv4()` به IPv4 رزولوو می‌شود (اسپوفر raw-socket به IP نیاز دارد). xray همچنان `127.0.0.1:40443` را صدا می‌زند و socks روی ۱۰۸۰۸ (برابر V2RayTun) است.
  - جریان نهایی: `xray (socks 10808) → 127.0.0.1:40443 (اسپوفر ما) → دامنهٔ CDN واقعی:443 (با DPI-bypass)` — دقیقاً الگوی هلپر داخلی V2RayTun.
- `core/xray_manager.py`: `real_server` حالا `upstream_address/upstream_port` را برمی‌گرداند (placeholder لوکال را به CDN واقعی رزولوو می‌کند).

### تمیزکاری UI لیست پروفایل (بازخورد «خیلی تنگ + بریدگی کلمات هنگام فعال‌شدن»)
- `ui/window.py`: لیست `setMinimumHeight(200)` + `setSpacing(6)` + `SingleSelection` + کف ارتفاع ردیف ۶۲px در `refresh()` (sizeHint قبل از layout کم گزارش می‌دهد و باعث بریدگی پیل «● فعال» می‌شد).
...
## 🔍 بازخورد ششم — افزودن لاگ تشخیصی به اسپوفر (2026-05-30)
**وضعیت کاربر:** با V2RayTun (که اسپوفر ما را روی 40443 صدا می‌زند) هم Tunnel و هم SNI Only وصل می‌شوند. اما با xray داخلیِ خودِ برنامه «هیچی وصل نمی‌شود». لاگ نشان می‌داد xray کانکشن‌ها را `accepted` می‌کند و به outbound `proxy` (→127.0.0.1:40443) می‌فرستد، اما **هیچ لاگی از اسپوفر** بعد از «Listening on 127.0.0.1:40443» دیده نمی‌شد.

### چرا تشخیص سخت بود
- کانفیگ تولیدشدهٔ `xray_config.json` کاملاً درست بود (outbound vless→127.0.0.1:40443، SNI واقعی، xhttp host/path).
- اسپوفر با V2RayTun کار می‌کند → پس WinDivert/درایور روی دستگاه کاربر سالم است و خودِ اسپوفر ایراد ندارد.
- متد `_handle` اسپوفر در مسیر موفق **هیچ لاگی نمی‌داد** و بین «connect» تا «t2a_event» هم ساکت بود؛ پس نمی‌شد فهمید کجا گیر می‌کند.

### رفع (`main.py` — مشاهده‌پذیری per-connection)
به `ProxyServer._handle` لاگ‌های دقیق per-connection اضافه شد:
- `[conn #N] از xray رسید (…) → اتصال به CONNECT_IP:PORT`
- `[conn #N] TCP وصل شد (مبدأ interface_ipv4:port) — منتظر تأیید ClientHello جعلی…`
- شکست اتصال upstream با متن خطای واقعی (`exc`).
- شکست/تایم‌اوت دست‌دادن جعلی با راهنمایی «آیا WinDivert/درایور با دسترسی Admin اجرا شده؟».
- `[conn #N] ✓ ClientHello جعلی تأیید شد — شروع رله`
- `[conn #N] رله پایان یافت (↑..B ↓..B)` برای دیدن این‌که آیا اصلاً داده جریان یافت.

این لاگ‌ها در بیلد بعدی دقیقاً نشان می‌دهند که آیا کانکشن‌های xray به اسپوفر می‌رسند، اتصال به CDN برقرار می‌شود، و دست‌دادن جعلی موفق است یا نه — تا ریشهٔ واقعی مشخص شود.

## 🎯 بازخورد هفتم — تشخیص ریشه: ترافیک وارد تونل نمی‌شود (2026-05-30)
**یافتهٔ کلیدی کاربر:** SNI Only بدون کانفیگ کار می‌کند؛ Tunnel هم وقتی V2RayTun باز باشد کار می‌کند؛ اما برنامهٔ ما **به‌تنهایی** (با xray داخلی، بدون V2RayTun) کار نمی‌کند. کاربر شک داشت xray داخلی به پورت 40443 وصل نمی‌شود.

### مقایسهٔ کانفیگ V2RayTun با کانفیگ ما (قطعی)
کاربر فایل کامل کانفیگ V2RayTun را فرستاد. outbound آن **بایت‌به‌بایت معادل** کانفیگ ماست:
- vless → `127.0.0.1:40443`، xhttp، host/path/mode یکسان، SNI واقعی یکسان.
- تنها تفاوت‌ها: ماژول DNS و routing غنی‌تر V2RayTun + فیلدهای cosmetic (`email`, `security:auto`).
→ **پس کانفیگ و هستهٔ xray ما مشکلی ندارند.** در لاگ بیلد ۷ هم xray ده‌ها کانکشن را `accepted` کرد.

### ریشهٔ واقعی
V2RayTun هنگام اتصال **پروکسی سیستم/TUN ویندوز** را ست می‌کند، پس کل ترافیک سیستم خودکار وارد SOCKS آن می‌شود. برنامهٔ ما `system_proxy=false` دارد و **ترافیک OS را capture نمی‌کند**؛ مگر کاربر دستی مرورگر را روی `127.0.0.1:10808` بگذارد یا «پروکسی سیستم» را روشن کند. در بیلد ۸ اصلاً هیچ `accepted` در لاگ نبود = هیچ ترافیکی به SOCKS 10808 نرسید = xray هرگز اسپوفر را صدا نزد.

### رفع‌ها (core/engine.py، core/config_store.py)
1. **خودآزمای مسیر داخلی (`_self_test_chain`)** — چند ثانیه بعد از Start، یک درخواست واقعی از طریق پورت HTTP محلی خودمان به `https://www.gstatic.com/generate_204` می‌زند و نتیجه را شفاف لاگ می‌کند:
   - `✓ مسیر داخلی سالم است` → تونل کار می‌کند؛ پس اگر مرورگر باز نمی‌کند یعنی پروکسی ست نشده.
   - `✗ مسیر داخلی شکست خورد: …` → xray↔اسپوفر یا اسپوفر↔CDN مشکل دارد.
   این آزمون مستقل از تنظیمات مرورگر کاربر است و ابهام «کار می‌کند یا نه» را برطرف می‌کند.
2. **راهنمای صریح هنگام Start** — اگر `system_proxy` خاموش باشد، یک خط راهنما لاگ می‌شود که چطور مرورگر/سیستم را روی SOCKS/HTTP محلی بگذارند.
